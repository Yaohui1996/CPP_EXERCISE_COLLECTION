...
GradeBook::GradeBook(const std::string &name)
    : courseName(name), aCount(0), bCount(0), cCount(0), dCount(0), fCount(0), maximumGrade(0)
{
}

GradeBook::GradeBook(const std::string &course_name, const std::string &teacher_name)
...
void GradeBook::setTeacherName(const std::string &name)
{
    teacherName = name;
}

const std::string &GradeBook::getTeacherName() const
{
    return teacherName;
}
