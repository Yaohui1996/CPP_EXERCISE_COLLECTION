...
public:
    Bulk_quote() = default;
    Bulk_quote(const std::string&, const double, const std::size_t, const double);
...
Bulk_quote::Bulk_quote(const std::string& book, const double p, const std::size_t qty, const double disc) : Quote(book, p), min_qty(qty), discount(disc) {}
