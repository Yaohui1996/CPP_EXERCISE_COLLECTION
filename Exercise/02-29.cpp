...
int main() {
  ...
  std::cout << "不合法, p3是指向常量的常量指针, 不能对p3的内容(储存的地址)进行修改" << std::endl;
  // const int ic = 100;
  // int b = 200;
  // const int *const p3 = &b;
  // p3 = &ic;
  std::cout << "编译器抛出: error: assignment of read-only variable 'p3'" << std::endl;
  // (e) p2 = p1;
  std::cout << "(e) p2 = p1;" << std::endl;
  std::cout << "不合法, p2是常量指针, 不能对p2的内容(储存的地址)进行修改" << std::endl;
  // int a = 100;
  // int *p1, *const p2 = &a;
  // p2 = p1;
  std::cout << "编译器抛出: error: assignment of read-only variable 'p2'" << std::endl;
  // (f) ic = *p3;
  ...
  // ic = *p3;
  std::cout << "编译器抛出: error: assignment of read-only variable 'ic'" << std::endl;
  return 0;
}